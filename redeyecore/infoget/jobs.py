...
def get_vsa_and_lobapp_data():
    ...
    # New empty list to hold lob agents missing the custom fields
    agents_missing_custom_field = []

    # VSA API URL to get ALL lob agents (all agents in the 'lob' group)
    vsa_all_lob_agents_url = "https://realserverurl.kaseya.net/API/v1.0/assetmgmt/agents?$filter=substringof('lob', MachineGroup)"
    ...
    ### VSA - ALL LOB AGENTS ###
    # Instantiate the vsa api request class and get ALL LOB agents
    vsa_all_lob_agents_request_object = infoget_classes.make_vsa_api_request(vsa_all_lob_agents_url, existing_token)
    vsa_all_lob_agents_result = vsa_all_lob_agents_request_object.request_result_all()
    vsa_all_lob_agents = vsa_all_lob_agents_result[1]
    logging.info("Executed 'vsa_all_lob_agents' call. Got HTTP {}".format(vsa_all_lob_agents_result[0]))
    ...
    # Loop through 'vsa_all_lob_agents' and add relevant values to 'data_for_insert' dict
    logging.info("Starting the 'for agent in vsa_all_lob_agents' loop.")
    for agent in vsa_all_lob_agents[1]:
        ...
        try:
            ...
            # Issue-002: Check for blank vsa_lac_device_name first, in case the custom fields are missing / blank
            if not vsa_lac_device_name:
                data_for_insert['lob_last_lob_query'] = None
            else:
                ...
                lob_last_lob_query_utc = lobapp_db_cursor.fetchall()

                for item in lob_last_lob_query_utc:
                    # Instantiate the time_convert class
                    lob_last_query_convert_object = infoget_classes.time_convert(item[0])
                    lob_last_lob_query_raw = lob_last_query_convert_object.lobapp_to_mysql()

                    # Check for database-side NULL values
                    if '1000-01-01 00:00:00' in lob_last_lob_query_raw:
                        lob_last_lob_query = None
                    else:
                        lob_last_lob_query = lob_last_lob_query_raw

                    # Append the LOB app site data to the insert dict for this agent
                    data_for_insert['lob_last_lob_query'] = lob_last_lob_query



            ####### 'lob_last_lob_update'
            # Get this agent's last lob code update (file download) timestamp from LOB app
            # Issue-002: Check for blank vsa_lac_device_name first, in case the custom fields are missing / blank
            # Issue-005 this was polling lac.last_queried instead of lac.last_checked
            if not vsa_lac_device_name:
                data_for_insert['lob_last_lob_update'] = None
            else:
                lobapp_last_lob_update_query = "SELECT CASE WHEN lac.last_checked IS NULL THEN '1000-01-01 00:00:00' ELSE lac.last_checked END FROM lob_app_computers lac WHERE lob.device_name = \'{}\'".format(vsa_lob_device_name)
                lobapp_db_cursor.execute(lobapp_last_lob_update_query)
                lobapp_last_lob_update_utc = lobapp_db_cursor.fetchall()

                for item in LOB app_last_lob_update_utc:
                    # Instantiate the time_convert class
                    lob_last_update_convert_object = infoget_classes.time_convert(item[0])
                    lob_last_lob_update_raw = lob_last_update_convert_object LOB app_to_mysql()

                    # Check for database-side NULL values
                    if '1000-01-01 00:00:00' in lob_last_lob_update_raw:
                        lob_last_lob_update = None
                    else:
                        lob_last_lob_update = lob_last_lob_update_raw

                    # Append the LOB app site data to the insert dict for this agent
                    data_for_insert['lob_last_lob_update'] = lob_last_lob_update
            ...
            if not vsa_lob_device_name:
                ...
                if '1000-01-01 00:00:00' in lob_last_lob_update_raw or '1000-01-01 00:00:00' in lob_last_lob_query_raw:
                ...
                # Added this smarter check on 10/17/2022 for feature-007
                    ...
                    # Get current timestamp and convert the lob_last_lob_query string back into a datetime object
                    now = datetime.utcnow()
                    lobts_dt = datetime.strptime(lob_last_lob_query, '%Y-%m-%d %H:%M:%S')

                    # Get the difference between now and lob_last_lob_query, in seconds
    ...
    try:
        ...
        #app_db_cursor.reset()
        query_string_0 = "DELETE FROM api_lobservers"
        ...
        for row in final_combined_data_to_insert:
            ...
            query_string_3 = "INSERT INTO api_lobservers (app_rmm_fetch_time, app_lob_fetch_time, rmm_agent_id, lob_computer_name, rmm_lac_computer_name, rmm_computer_name, rmm_agent_name, lob_location, rmm_lac_file_type, rmm_last_reboot, rmm_last_checkin, lob_last_lob_update, lob_last_lob_query, rmm_ram_mbytes, rmm_cpu_type, rmm_agent_is_offline, lob_agent_is_offline, app_run_number, rmm_uptime_score, lob_uptime_score) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
            ...
            app_db_cursor.execute(query_string_1, (row['app_rmm_fetch_time'], row['app_lob_fetch_time'], row['rmm_agent_id'], row['lob_computer_name'], row['rmm_lac_computer_name'], row['rmm_computer_name'], row['rmm_agent_name'], row['lob_location'], row['rmm_lac_file_type'], row['rmm_last_reboot'], row['rmm_last_checkin'], row['lob_last_lob_update'], row['lob_last_lob_query'], row['rmm_ram_mbytes'], row['rmm_cpu_type'], row['rmm_agent_is_offline'], row['lob_agent_is_offline'], row['app_run_number'], row['rmm_uptime_score'], row['lob_uptime_score']))
